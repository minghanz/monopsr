...
bool eval(string gt_dir, string result_dir, Mail* mail){
  ...
  string plot_dir       = result_dir + "/plot_low_iou";
  ...
  // print global step taken from folder name
  size_t i = result_dir.rfind('/', result_dir.length());
  if (i != string::npos) {
    mail->msg(result_dir.substr(i+1, result_dir.length() - i).c_str());
  }
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    ...
    if (eval_image[c]) {
      fp_det = fopen((result_dir + "/stats_" + CLASS_NAMES[c] + "_detection_low_iou.txt").c_str(), "w");
      if(compute_aos)
        fp_ori = fopen((result_dir + "/stats_" + CLASS_NAMES[c] + "_orientation_low_iou.txt").c_str(),"w");
